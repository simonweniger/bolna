...
class DeepgramTranscriber(BaseTranscriber):
    ...
    def __process_utterance_end_event(self, msg = None):
        logger.info("Transcriber Latency: {} for request id {}".format(time.time() - self.audio_submission_time,
        ...
        if msg == None:
            #This is due to speech final in nitro mode
            self.meta_info["utterance_end"]  = time.time()
        else:
        ...
        return create_ws_data_packet(self.finalized_transcript, self.meta_info)
        

    async def __process_speech_started_event(self):
        ...
        return create_ws_data_packet("TRANSCRIBER_BEGIN", self.meta_info)
        ...
        async for msg in ws:
            try:
                ...
                if msg["type"] == "UtteranceEnd":
                    utterance_end_packet = self.__process_utterance_end_event(msg)
                    if utterance_end_packet is not None:
                        yield utterance_end_packet
                    self.curr_message = ""
                    self.finalized_transcript = ""
                    continue
                ...
                if msg["type"] == "SpeechStarted":
                    transcriber_begin_packet = await self.__process_speech_started_event()
                    yield transcriber_begin_packet
                ...
                if self.process_interim_results == "true":
                    if msg["is_final"] is True and msg['speech_final'] is False:
                        ...
                        if transcript.strip() != self.curr_message.strip():
                    ...
                    elif msg["is_final"] is False and msg['speech_final'] is False: 
                        ...
                        package = self.__process_utterance_end_event()
                        yield package
